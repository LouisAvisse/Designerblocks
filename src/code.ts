...
let activityData: Record<string, number> = {}; // Date -> Count (File-wide)
let uiIsReady = false;
const pageStatsCache = new Map<string, Stats>();
...
function loadFromFile(key: string): any {
  try {
...
function initActivity() {
  ...
  for (const k in activityData) {
    if (k < cutoffKey) delete activityData[k];
  }
}
...
function recordActivity(amount: number = 1) {
  const key = getTodayKey();
  activityData[key] = (activityData[key] ?? 0) + amount;
...
async function crawlPage(page: PageNode) {
...
async function startCrawler() {
  ...
  try {
    ...
    try {
      await crawlPage(figma.currentPage);
    } catch (e) {
...
function sendAllStats() {
  ...
  for (const s of pageStatsCache.values()) {
    tN += s.nodes; tF += s.frames; tC += s.components;
  }
...
function updateSelection() {
  ...
  sel.forEach(n => types[n.type] = (types[n.type] ?? 0) + 1);
  figma.ui.postMessage({
